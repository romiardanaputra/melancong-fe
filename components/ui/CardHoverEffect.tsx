      ...
      {items.map((item, idx) => (
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className='absolute inset-0 block h-full w-full rounded-3xl bg-cyan-300/[.8] dark:bg-slate-800/[0.8]'
...
export const CardTitle = ({
  ...
  return (
    <h4 className={cn('mt-4 font-bold tracking-wide text-zinc-900', className)}>
      {children}
    </h4>
  )
}
